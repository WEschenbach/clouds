# x=Data; y = NULL; plot.type = c("multiple", "single")
# panel = lines;  yax.flip = FALSE;
# mar.multi = c(0,5.1, 0, if (yax.flip) 5.1 else 2.1);
# oma.multi = c(6,0, 5, 0); axes = TRUE; keyspan=.4;
# drawyears=TRUE;printloess=TRUE



plot.ts2=function (x, y = NULL, plot.type = c("multiple", "single"), xy.labels,
                   ...
                   drawyears=TRUE,printloess=TRUE, ...)
{
  plotts <- function(x, y = NULL,
                     plot.type = c("multiple","single"), xy.labels, xy.lines, panel = lines, nc, xlabel,
                     ylabel, type = "l", xlim = NULL, ylim = NULL,
                     xlab = "Time",
                     ...
                     printloess=printloess, drawyears=drawyears, main = NULL, ...) {
    ...
    if (plot.type == "multiple" && nser > 1) {
      ...
      on.exit(par(oldpar))
      linewidth=2
      collist=c("red4","darkcyan","gray50")
      par(mgp=c(2,1,0))
      for (i in 1L:nser) {

        plot.default(x[, i], axes = FALSE, xlab = "",
                     ylab = "", log = log, col = col, bg = bg, pch = pch,
                     ann = ann, type = "n", yaxp=c(-2,2,4), ylim=ylim, ...)
        panel(x[, i], col = col, bg = bg, pch = pch, lwd=linewidth,
              type = type, ...)
        abline(h=mean(x[,i],na.rm=TRUE),col="red3",lty=c("dotted","solid","dotted"))
        if (drawyears==TRUE){
          abline(v=1900:2020,col="skyblue2",lty="dashed",lwd=1.5)
          abline(v=1900.5:2020.5,col="skyblue",lty="dotted",lwd=1.75)
        }
        if (!is.na(majorlines[1])){
          abline(v=majorlines,col="firebrick4",lwd=1.75,lty="dashed")
        }

        lines(x[, i], col = collist[i], bg = bg, pch = 20, cex=.8,lwd=linewidth,
              type = "o", ...)
        if ((i==nser) & (printloess==TRUE)) {
          myloess=loess(x[,i]~as.vector(time(x[,i])),span=keyspan)

          lines(myloess$fitted~as.vector(time(x[,i])),col="deepskyblue2",lwd=3)

          abline(h=mean(x[,i])+1*sd(x[,i]),col="gold3",lwd=2,lty="dotted")
          abline(h=mean(x[,i])-1*sd(x[,i]),col="gold3",lwd=2,lty="dotted")
        }
      if (frame.plot)
          box(...)
  ...
  plotts(x = x, y = y, plot.type = plot.type, xy.labels = xy.labels,
         ...
         ylabel = ylabel, axes = axes, printloess=printloess, drawyears=drawyears, ...)
}
